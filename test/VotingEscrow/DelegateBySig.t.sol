...
contract DelegateBySig is VotingEscrowTest {
  ...
  function createDelegationSignature(Vm.Wallet memory delegator, address delegate, uint256 nonce, uint256 expiry) public returns (uint8 v, bytes32 r, bytes32 s) {
    bytes32 domainSeparator = keccak256(abi.encode(votingEscrow.DOMAIN_TYPEHASH(), keccak256(bytes(votingEscrow.name())), keccak256(bytes(votingEscrow.version())), block.chainid, address(votingEscrow)));
    ...
    function testFuzz_delegatebysig_invalidNonce(address pranker, address delegate, uint256 expiry, uint256 nonce) public {
      vm.assume(pranker != address(0));
      vm.assume(delegate != address(0));
      vm.assume(pranker != delegate);
      vm.assume(expiry > block.timestamp);
    ...
    function testFuzz_delegatebysig_invalidSignature(address pranker, address delegate, uint256 expiry, uint8 v, bytes32 r, bytes32 s) public {
      vm.assume(pranker != address(0));
      vm.assume(delegate != address(0));
      vm.assume(pranker != delegate);
      vm.assume(expiry > block.timestamp);
