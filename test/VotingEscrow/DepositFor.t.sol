...
contract DepositFor is VotingEscrowTest {
  ...
  uint internal constant WEEK = 7 * 86400;
    ...
    function testFuzz_depositFor_simple(address pranker, uint256 amount, uint256 secondAmount, uint256 duration) public {
    ...
    function testFuzz_depositFor_Expired(address pranker, uint256 amount, uint256 secondAmount, uint256 duration) public {
      vm.assume(pranker != address(0));
      amount = bound(amount, 1, 10e25);
      secondAmount = bound(secondAmount, 1, 10e25);
      duration = bound(duration, 7 * 86400, MAXTIME);
