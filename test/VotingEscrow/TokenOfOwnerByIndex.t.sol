...
contract TokenOfOwnerByIndex is VotingEscrowTest {
  ...
  uint internal constant MAXTIME = 2 * 365 * 86400;
    ...
    function testFuzz_tokenOfOwnerByIndex_multiple(address pranker, uint256 amount, uint256 duration, uint8 nftNumber) public {
    ...
    function testFuzz_tokenOfOwnerByIndex_InvalidIndex(address pranker, uint256 index, uint256 amount, uint256 duration) public {
      vm.assume(pranker != address(0));
      amount = bound(amount, 1, 10e25);
      duration = bound(duration, 7 * 86400, MAXTIME);
      vm.assume(index != 0);
