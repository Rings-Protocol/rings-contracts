...
contract GetLastUserSlope is VotingEscrowTest {
  ...
  uint internal constant MAXTIME = 2 * 365 * 86400;
    ...
    function testFuzz_getLastUserSlope_LockStart(address pranker, uint256 amount, uint256 duration) public {
      ...
      assertEq(votingEscrow.get_last_user_slope(tokenId), SafeCastLib.toInt128(amount / MAXTIME), "Value should be amount / max duration");
    }

    function testFuzz_getLastUserSlope_LockMiddle(address pranker, uint256 amount, uint256 duration, uint8 waitWeeks) public {
      vm.assume(pranker != address(0));
      amount = bound(amount, 1, 10e25);
      duration = bound(duration, 7 * 86400, MAXTIME);
